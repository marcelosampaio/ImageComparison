...
//  ImageComparison
//
//  Created by Marcelo on 28/12/18.
...
class PersistenceManager {
    ...
    public func addFile(id: String, data: NSData) -> (absolutePath: String?, error: NSError?) {
        ...
        let pathFileName = self.pathComposer(id: id)
        
        print("👉 path file name: \(pathFileName)")
        
        if fileExists(pathFileName) {
            return(nil, self.generateError(code: 9001, message: "arquivo-existente"))
        }
        ...
        do {
            ...
            if (dataSize > deviceRemainingFreeSpaceInBytes()) {
                let errorDomain = "SNR"
                let errorCode = 8000
                let errorUserInfo = [NSLocalizedDescriptionKey : "Falta de espaço para armazenamento."]
            ...
            let errorDomain = "SNR"
            let errorCode = 8000
            let errorUserInfo = [NSLocalizedDescriptionKey : "erro interno"]
            let errorObject = NSError.init(domain: errorDomain, code: errorCode, userInfo: errorUserInfo)
    ...
    // MARK: - Error Handler
    public func generateError(code: Int, message: String) -> NSError {
        let errorDomain = "SNR"
        let errorCode = code
